...
class Servicio_Contratado(Base):
    ...
    id_publicacion = Column(
        Integer,
        ForeignKey("publicacion_servicio.id_publicacion", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    estado_servicio = Column(
        String(20), nullable=False, default="contactado"
    )  # contactado, confirmado, en_proceso, finalizado, cancelado
    ...
    proveedor_servicio = relationship(
        "Proveedor_Servicio", back_populates="servicio_contratado"
    )
    publicacion_servicio = relationship(
        "Publicacion_Servicio", back_populates="servicio_contratado"
    )
    reseña_servicio = relationship(
        "Reseña_Servicio", back_populates="servicio_contratado", cascade="all, delete"
    )
    alerta_sistema = relationship(
        "Alerta_Sistema", back_populates="servicio_contratado", cascade="all, delete"
    )
