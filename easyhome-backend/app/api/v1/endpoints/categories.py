...
async def get_categories(db: Session = Depends(get_db)):
    ...
    try:
        categories = (
            ...
            .order_by(Categoria_Servicio.orden_visualizacion)
            .all()
        )
        return categories
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error al obtener categorías: {str(e)}"
        )
...
async def create_category(
    ...
    try:
        ...
        existing = (
            db.query(Categoria_Servicio)
            .filter(Categoria_Servicio.nombre_categoria == category.nombre_categoria)
            .first()
        )
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Error al crear categoría: {str(e)}"
        )
...
async def delete_category(
    ...
    try:
        category = (
            db.query(Categoria_Servicio)
            .filter(Categoria_Servicio.id_categoria == category_id)
            .first()
        )
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Error al eliminar categoría: {str(e)}"
        )
...
async def update_category(
    ...
    try:
        ...
        if (
            category_update.nombre_categoria
            and category_update.nombre_categoria != category.nombre_categoria
        ):
            existing = (
                db.query(Categoria_Servicio)
                .filter(
                    Categoria_Servicio.nombre_categoria
                    == category_update.nombre_categoria
                )
                .first()
            )
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Error al actualizar categoría: {str(e)}"
        )
